...
export class TruckListController {
    ...
    async getAllTruckLists(req: Request, res: Response): Promise<void> {
        const truckLists = await this.truckListService.findAll();
        res.json(truckLists);
    }

    async getTruckListById(req: Request, res: Response): Promise<void> {
    ...
    async createTruckList(req: Request, res: Response): Promise<void> {
        const newTruckList = await this.truckListService.create(req.body);
        res.status(201).json(newTruckList);
    }

    async updateTruckList(req: Request, res: Response): Promise<void> {
        const id = parseInt(req.params.id);
    ...
    async deleteTruckList(req: Request, res: Response): Promise<void> {
        const id = parseInt(req.params.id);
